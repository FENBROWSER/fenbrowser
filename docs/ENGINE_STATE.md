...
| Missing API observation | TESTED | Schema-v2 tracker resolves concrete HTML receivers and checked-in IDL evidence, preserves ordered read/write plus assignment timing, and exports bounded rich records after logger drain; fresh Google proves 5 page expandos and leaves 8 reads unclassified | Prototype/descriptor evidence remains for the read-only protocol/probe observations |
...
| CSS/style/layout/paint | REGRESSION_PROTECTED | Google renders at 1280x800; WebDriver geometry refreshes current layout/styles, and render generations reject older DOM/style/telemetry publication after replacement navigation | Broader cross-frame and renderer-process generation coverage remains |
...
| Deterministic first-causal-blocker classifier | TESTED | `first_blocker.json` models 19 ordered milestones, rejects missing artifacts and inverted evidence clocks as verification insufficiency, and `20260716T120932Z` reports `none` with lifecycle/DOM/frame/artifact agreement | Schema-version and malformed-artifact fixture coverage remains |
...
| Missing API bundle schema | TESTED | The bundle retains up to 512 ordered schema-v2 records with first/observed read/write/descriptor/property-check kinds, descriptor target identity, assignment timing, bounded boolean function-prototype marker evidence, and checked-in IDL stringifier/partial-interface resolution; `20260716T115831Z` classifies all 25/25 records | Explicit prototype-operation coverage remains; confirmed missing members still require causal selection |
...
Google remains the current target because it crosses the boot and interaction pipeline. The 2026-07-15 passive run loaded the document, completed 18 script executions without a direct script failure, fired lifecycle events, built DOM/style/layout/paint state, and rendered the main UI. The earlier timer and Promise host-object defects remain regression-protected, and the current interaction bundle reports zero callback failures and zero exceptions. `logs/real-site/www.google.com/20260715T103925Z/` uses the generic Tooling command at 1280x800 to hit-test `TEXTAREA#APjFqb`, focus it, accept nonce `fen715j`, dispatch the expected input/change/blur/click/submit sequence, complete a 200 GET `/search` navigation, and follow Google's delayed redirect to a genuine HTTP 429 `/sorry/` challenge. Render generations prevent older detached CSS/script work from republishing DOM, styles, repaint state, loading completion, or telemetry after replacement navigation. Terminal lifecycle, active DOM/rendered text, and the visibly different after screenshot all describe navigation 3's challenge document; `first_blocker.json` is `none`, and no security challenge was bypassed. Normal search-result rendering is externally blocked by the challenge. Missing-API classification remains separately open.

Evidence: `logs/real-site/www.google.com/20260715T103925Z/`.
