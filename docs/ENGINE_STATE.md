...
| Missing API observation | TESTED | Schema-v2 tracker resolves embedded checked-in IDL inheritance/receiver evidence, records first arbitrary writes as page expandos, keeps unknown reads out of standards priority, and exports bounded rich records after logger drain | Prototype/descriptor evidence and fresh Google reclassification remain |
...
| Missing API bundle schema | TESTED | The bundle retains up to 512 ordered schema-v2 records with expando writes and full receiver/source/script/navigation provenance, plus explicit truncation counts | Prototype/descriptor operation evidence and a fresh Google bundle remain |
