...
| Missing API observation | TESTED | Schema-v2 tracker resolves embedded checked-in IDL inheritance/receiver evidence, records first arbitrary writes as page expandos, and keeps unknown reads out of standards priority | Prototype/descriptor evidence and rich bundle merging remain |
...
| Missing API bundle schema | STUBBED | Compact records classify local standard, legacy, and unknown reads; the sidecar also preserves expando writes and full receiver/source/navigation provenance | Rich sidecar records are not yet merged into the run bundle |
