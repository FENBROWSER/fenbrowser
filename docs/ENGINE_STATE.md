...
| CSS/style/layout/paint | TESTED | Google renders at the diagnostic 1280x800 viewport; WebDriver geometry refreshes current layout/styles before hit testing | After the terminal 429 challenge navigation, the active root/text/frame incorrectly retain the prior homepage document |
...
| Deterministic first-causal-blocker classifier | TESTED | `first_blocker.json` models 19 ordered milestones, fixture tests remain green, and the terminal Google navigation reports `none` | It does not yet detect the contradiction between terminal lifecycle/DOM artifacts and stale active root/text/frame evidence |
...
Google remains the current target because it crosses the boot and interaction pipeline. The 2026-07-15 passive run loaded the document, completed 18 script executions without a direct script failure, fired lifecycle events, built DOM/style/layout/paint state, and rendered the main UI. The earlier timer and Promise host-object defects remain regression-protected, and the current interaction bundle reports zero callback failures and zero exceptions. `logs/real-site/www.google.com/20260715T102729Z/` uses the generic Tooling command at 1280x800 to hit-test `TEXTAREA#APjFqb`, focus it, accept nonce `fen715i`, dispatch the expected input/change/blur/click/submit sequence, complete a 200 GET `/search` navigation, and follow Google's delayed redirect to a genuine HTTP 429 `/sorry/` challenge. Terminal-generation quieting correctly retains navigation 3 and `first_blocker.json` reports `none`; no security challenge was bypassed. The terminal lifecycle and DOM artifact describe the challenge, while the active root, rendered text, and after frame still expose the prior homepage. Preventing stale cross-navigation render-state publication is the next acceptance gap. Missing-API classification remains separately open.

Evidence: `logs/real-site/www.google.com/20260715T102729Z/`.
