...
| Missing API observation | TESTED | Schema-v2 tracker and compact bundle projection classify evidence-backed dispositions; unknown reads default to `UNCLASSIFIED` and are not standards-priority eligible | Assignment/prototype/checked-in-IDL evidence and rich bundle merging remain |
...
| Missing API bundle schema | STUBBED | Compact records include classification, operation, reason, and priority eligibility; local legacy/unknown fixture is correct | Script/navigation/source/receiver timing provenance still comes only from the sidecar tracker |
