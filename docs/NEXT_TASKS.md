...
| github.com | 2891 nodes | 1920/2891 styled | 69/84 scripts | Captured | 1418 layout boxes; 64 zero-area boxes; header height fixed/top-aligned; nav labels no longer self-overlap; hero headline/description wrap inside max-width caps; nav allocation and visual sizing remain |
...
- **Current**: GitHub captures layout but still has 64 zero-area boxes after unitless `line-height` flex sizing, repeated percentage-height normalization, auto-height positioned-header percentage sizing, flex/grid item blockification, shrink-to-fit text/row-flex intrinsic width preservation, leaf/control row-flex shrink floors, and inline max-width line limits were fixed. `height:100%` under an auto-height positioned header no longer resolves against viewport/out-of-flow used geometry; the header is now 60px and top-aligned. Column-flex hero text now wraps inside its resolved max-width cap. Remaining visual issues are nav allocation against search/sign-in controls, collapsed dropdown widths, and hero visual/carousel sizing, not header viewport-height inflation, nav labels shrinking below their own text, or the hero headline overflowing as one unwrapped line.
...
1. `style_layout.json` - 64 zero-area boxes, no captured layout blocker
2. `layout_dump.txt` - header is `1280x60` and top-aligned; hero headline/description wrap inside max-width caps; inspect remaining nav allocation against search/sign-in controls, collapsed dropdown widths, and visual/carousel geometry
3. `screenshot.png` - nav labels no longer self-overlap and hero text no longer clips, but search/sign-in overlap and hero visual sizing remain the visual acceptance signal
