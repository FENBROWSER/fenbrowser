...
2. `layout_dump.txt` - header is `1280x60` and top-aligned; logo shell is shrink-wrapped; hero headline/description wrap inside max-width caps; inspect collapsed dropdown/background widths, sign-in content-box sizing, and visual/carousel geometry
3. `screenshot.png` - nav labels no longer self-overlap, search/sign-in/sign-up are visibly allocated, and hero text no longer clips; remaining visual acceptance signal is dropdown/background and hero visual sizing
