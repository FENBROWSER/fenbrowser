...
| Missing API runtime tracker | TESTED | Schema-v2 per-site records and the post-drain bounded bundle include concrete HTML receiver identity, assignment-before-read, checked-in-IDL inheritance/receiver evidence, classification, operation, reason, priority eligibility, and source/navigation provenance; fresh Google retains 25/25 classified records | Add prototype/descriptor operation evidence for the 13 unclassified records |
