...
5. Run `debug-site` with an explicit settle value. For interaction acceptance, run `debug-site-interact <url> <target_selector> <text> <submit_selector> <settle_ms> <interaction_settle_ms>` after the passive bundle is stable. The diagnostic host and screenshots use the same fixed 1280x800 viewport; do not compare geometry from a differently sized host.
