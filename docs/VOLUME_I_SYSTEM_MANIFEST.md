...
- **Paint/Compositing Baseline (PC-1, 2026-02-20)**:
  ...
  - PC-1.20 Google noscript-warning suppression in fallback promotion (2026-04-17): `CustomHtmlEngine` now suppresses hidden fallback-block promotion on Google hosts, preventing the “If you’re having trouble accessing Google Search…” warning from being force-shown when JS-heavy-page safe mode is active. Regression coverage added in `FenBrowser.Tests/Engine/CustomHtmlEngineFallbackPromotionTests.cs`.
  - PC-1.21 Google safe-mode bypass for white-page prevention (2026-04-17): `CustomHtmlEngine` now bypasses no-JS safe-mode forcing (`ShouldPreferFallbackDom` and `IsJsHeavyAppShell`) on Google hosts so Google pages are not downgraded into fallback-only rendering paths that can surface blank/white page states after warning suppression. Regression coverage added in `FenBrowser.Tests/Engine/CustomHtmlEngineGoogleSafeModeBypassTests.cs`.
  - PC-1.22 Google fallback-banner DOM stripping (2026-04-17): `CustomHtmlEngine` now explicitly removes Google access-trouble fallback banner blocks from the no-JS sanitization path (matching “trouble accessing Google Search” + fallback action text), preventing reappearance of the warning copy even when it is emitted as visible DOM content rather than hidden promoted fallback. Regression coverage extended in `FenBrowser.Tests/Engine/CustomHtmlEngineFallbackPromotionTests.cs`.
  - PC-1.23 Google `yvlrue` unhide-script suppression (2026-04-17): `CustomHtmlEngine` now strips Google challenge trouble-banner artifacts (`div#yvlrue` and inline scripts that unhide it via `cad=sg_trbl` / `cssId='yvlrue'`) before JS execution, preventing the delayed fallback warning from resurfacing on Google `/search` result URLs after initial render. Regression coverage extended in `FenBrowser.Tests/Engine/CustomHtmlEngineFallbackPromotionTests.cs`.
