...
- Class-element parser coverage now accepts parser-only computed names, private methods/accesses, class fields, and async/generator method forms while `BytecodeCompiler` rejects those elements with `FeatureSupportLevel.ParserOnly` before runtime lowering. This is covered by `ParserTests`, `ObjectAndBytecodeTests`, exact `built-ins/Function/prototype/toString` parser-subset recheck at `80/80`, and refreshed `--parser-subset --max 10000` at `9992/10000` with only invalid runtime-negative configs remaining.
