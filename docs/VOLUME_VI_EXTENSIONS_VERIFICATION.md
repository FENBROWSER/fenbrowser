...
- Broader class parser-subset coverage (`language/statements/class`, `--max 600`) now closes all non-decorator failures in this window, including strict class-name identifiers, generator/async-generator `yield` early errors, computed-name `|=` assignment parsing, and async-generator rest-pattern early errors. Current status is `588/600`; the remaining 12 failures are all decorator syntax (`@`) cases.
