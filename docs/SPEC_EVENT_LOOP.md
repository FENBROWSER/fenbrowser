...
sequenceDiagram
    ...
    loop Every Tick
        EventLoop->>TaskQueue: Dequeue Task
        alt Has Task
            TaskQueue-->>EventLoop: Execute Task
            EventLoop->>MicrotaskQueue: Checkpoint()
            loop Until Empty
                MicrotaskQueue-->>EventLoop: Drain Microtasks
            end
        end

        EventLoop->>Render: Check Layout Dirty?
        alt Is Dirty
            Render-->>Render: Measure -> Layout -> Paint
        end

        EventLoop->>EventLoop: Next Tick
    end
```
