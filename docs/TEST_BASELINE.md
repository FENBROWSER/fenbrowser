...
| Lifecycle transition-detail and classifier slice | `dotnet test FenBrowser.Tests/FenBrowser.Tests.csproj -c Release --no-build --no-restore --filter "FullyQualifiedName~BrowserLifecycleDetailTests|FullyQualifiedName~NavigationLifecycleTrackerTests|FullyQualifiedName~FirstBlockerClassifierTests" --logger "console;verbosity=minimal"` | 20 passed, 0 failed, 0 skipped; missing-artifact and clock-inversion cases included and listed | REGRESSION_PROTECTED |
...
| --- | --- |
| `MissingApiTrackerTests` | yes; 17 tests |
...
| Function-prototype marker local fixture | `logs/real-site/file_c_users_udayk_videos_fenbrowser-test_fenbrowser.tests_fixtures_diagnostics_missing_api_function_prototype_marker.html/20260716T113659Z/` | One missing host read carries bounded boolean function-prototype-marker evidence and is `SITE_EXPANDO`; 0 callback failures; 0 exceptions; `first_blocker.json` is `none`; 26/26 artifacts | REGRESSION_PROTECTED |
| Missing-property operation-kind local fixture | `logs/real-site/file_c_users_udayk_videos_fenbrowser-test_fenbrowser.tests_fixtures_diagnostics_missing_api_operation_kinds.html/20260716T114816Z/` | Renders `true|false|false`; records descriptor, `in`, and own-property checks as `DESCRIPTOR_OPERATION`, `IN_CHECK`, and `DESCRIPTOR_OPERATION`; 0 callback failures; 0 exceptions; blocker `none`; 26/26 artifacts | REGRESSION_PROTECTED |
| Stringifier/partial-interface classification fixture | `logs/real-site/file_c_users_udayk_videos_fenbrowser-test_fenbrowser.tests_fixtures_diagnostics_missing_api_stringifier_partial_interface.html/20260716T115734Z/` | Renders `undefined|undefined`; both missing reads are receiver-matched standards records without changing runtime exposure; 0 callback failures; 0 exceptions; blocker `none`; 26/26 artifacts | REGRESSION_PROTECTED |
| Google passive classification | `logs/real-site/www.google.com/20260716T120932Z/` | Main UI visible; all 25 missing-property records classify as 11 standard, 11 site expandos, 2 wrong receivers, and 1 legacy probe; 0 callback failures; 0 exceptions; blocker `none`; 0 evidence-quality warnings; 26/26 artifacts | TESTED |
