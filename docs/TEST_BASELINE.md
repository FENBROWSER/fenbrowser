...
| Missing-API schema-v2 classification | `dotnet test FenBrowser.Tests/FenBrowser.Tests.csproj -c Release --no-build --filter "FullyQualifiedName~MissingApiTrackerTests|FullyQualifiedName~DebugSiteMissingApiClassificationTests" --logger "console;verbosity=minimal"` | 6 passed, 0 failed, 0 skipped | REGRESSION_PROTECTED |
