...
| --- | --- | --- | --- |
| Google interaction | `logs/real-site/www.google.com/20260715T102729Z/` | At 1280x800, hit-tested `TEXTAREA#APjFqb`, focused it, accepted nonce `fen715i`, emitted keyboard/input/change/blur plus submit-control pointer/click and form submit records, completed a 200 GET `/search` navigation, then followed Google's delayed redirect to a genuine HTTP 429 `/sorry/` challenge. Lifecycle and the DOM artifact reached navigation 3, `first_blocker.json` reports `none`, and callback/exception counts are zero. The active root, rendered-text sample, and after screenshot incorrectly retain the submitted homepage document, exposing stale render-state publication across navigation; no challenge bypass was attempted. | TESTED |
...
| WebDriver interaction smoke | REGRESSION_PROTECTED | Local click/type/canceled-and-successful-submit, current-layout hit testing, compound selector lookup, chained-navigation settlement, and Tooling viewport/artifact ownership are covered; Google focus/type/submit/navigation is captured, while active result-document/frame agreement remains required |
