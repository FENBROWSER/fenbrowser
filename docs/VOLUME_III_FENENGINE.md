...
- The FenJS browser bridge now also exposes `document.createTextNode(...)`, `document.createComment(...)`, and `document.createDocumentFragment(...)`, plus the corresponding `CharacterData` mutators (`appendData`, `insertData`, `deleteData`, `replaceData`, `substringData`) and fragment append/query helpers, so text/comment/fragment-driven DOM assembly can stay on the FenJS path.
...
- `FenBrowser.FenEngine/Rendering/CustomHtmlEngine.cs`
  ...
  - Script-created iframe `src` mutations now route through the browser frame loader, attach the parsed frame `Document` under the iframe, run the frame scripts against the frame root, and let iframe subdocument CSS resolve against the frame document base URI.
  - Incremental recascade now traverses `Document` / `DocumentFragment` nodes under iframe elements and uses each dirty root's owning document URI, so script-inserted frame descendants can receive their frame-local styles on the follow-up render.
