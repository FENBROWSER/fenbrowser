...
- `docs/test262_results.md` is the **committed source of truth** for per-category pass rates (regenerated by `scripts/test262_report.py` from the local batched store). **To check status, READ that file — do not re-run the full ~53k suite.** A full run is for occasional rebaselining only.
- Work loop: pick a below-95% category from that file's worklist → find the shared root cause in `Results/test262/batched/b_<tag>.json` (`failures[].details`) → fix the engine → **re-run only that category** with `bash scripts/rerun-test262-category.sh <category>` (overwrites its `b_<tag>.json` and regenerates the doc) → **commit the updated `docs/test262_results.md` with the fix**.
