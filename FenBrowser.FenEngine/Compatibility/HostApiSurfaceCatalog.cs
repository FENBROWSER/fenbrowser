...
"Scripting/JavaScriptEngine.cs",
                "Legacy JavaScriptEngine crypto bridge now provides digest(), generateKey(), importKey(), exportKey(), deriveBits(), deriveKey(), wrapKey(), unwrapKey(), encrypt(), decrypt(), sign(), and verify() for HMAC, AES-GCM, AES-CBC, AES-CTR, RSA-OAEP, RSA-PSS, ECDSA, ECDH, PBKDF2, HKDF, and RSASSA-PKCS1-v1_5 (65537 exponent), with fail-closed usage/extractability/parameter checks and deterministic thenable settlement; non-HKDF/PBKDF2 derive families remain pending."),
