...
public sealed class Test262ExpectationsTests
{
    ...
    [Fact]
    public void Load_AllowsUppercaseMilestonePrefix()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), "fenjs-test262-expectations-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var filePath = Path.Combine(tempRoot, "uppercase-milestone.json");
        ...
        try
        {
            ...
            var loaded = Test262Expectations.Load(filePath);
            Assert.Single(loaded.Entries);
            Assert.Equal("M2", loaded.Entries[0].ExpiresAtMilestone);
        }
        finally
        {
            Directory.Delete(tempRoot, recursive: true);
    ...
    [Fact]
    public void Load_RejectsMultiDotMilestoneFormat()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), "fenjs-test262-expectations-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var filePath = Path.Combine(tempRoot, "multidot-milestone.json");
    ...
    [Fact]
    public void Load_RejectsMilestoneWithoutNumericSuffix()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), "fenjs-test262-expectations-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var filePath = Path.Combine(tempRoot, "bare-milestone.json");
