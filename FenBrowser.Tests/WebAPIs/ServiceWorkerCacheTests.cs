...
namespace FenBrowser.Tests.WebAPIs
{
    public class ServiceWorkerCacheTests : IDisposable
    {
        ...
        {
            var openPromise = _cacheStorage.Get("open").AsFunction().Invoke(new FenValue[] { FenValue.FromString("v1") }, null).AsObject() as FenObject;
            ...
            // Verify persistence
            var hasPromise = _cacheStorage.Get("has").AsFunction().Invoke(new FenValue[] { FenValue.FromString("v1") }, null).AsObject() as FenObject;
        ...
        public async Task Cache_Put_StoresResponse()
        {
            ...
            // Put
            var putPromise = cache.Get("put").AsFunction().Invoke(new FenValue[] { request, FenValue.FromObject(response) }, null).AsObject() as FenObject;
            await AwaitPromise(putPromise);

            // Match
            var matchPromise = cache.Get("match").AsFunction().Invoke(new FenValue[] { request }, null).AsObject() as FenObject;
        ...
        public async Task Cache_Delete_RemovesEntry()
        {
            ...
            // Put
            await AwaitPromise(cache.Get("put").AsFunction().Invoke(new FenValue[] { request, FenValue.FromObject(response) }, null).AsObject() as FenObject);

            // Delete
            var deletePromise = cache.Get("delete").AsFunction().Invoke(new FenValue[] { request }, null).AsObject() as FenObject;
        ...
        {
            // Create cache
            await AwaitPromise(_cacheStorage.Get("open").AsFunction().Invoke(new FenValue[] { FenValue.FromString("delete-me") }, null).AsObject() as FenObject);

            // Verify exists
            var hasResult = await AwaitPromise(_cacheStorage.Get("has").AsFunction().Invoke(new FenValue[] { FenValue.FromString("delete-me") }, null).AsObject() as FenObject);
            Assert.True(((FenValue)hasResult).AsBoolean());

            // Delete
            var deleteResult = await AwaitPromise(_cacheStorage.Get("delete").AsFunction().Invoke(new FenValue[] { FenValue.FromString("delete-me") }, null).AsObject() as FenObject);
            Assert.True(((FenValue)deleteResult).AsBoolean());

            // Verify gone
            var hasResult2 = await AwaitPromise(_cacheStorage.Get("has").AsFunction().Invoke(new FenValue[] { FenValue.FromString("delete-me") }, null).AsObject() as FenObject);
            Assert.False(((FenValue)hasResult2).AsBoolean());
        }

        private async Task<FenValue> AwaitPromise(FenObject promise)
        {
