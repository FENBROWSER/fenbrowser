...
namespace FenBrowser.Tests.Engine
{
    ...
    public class JsCryptoCompatibilityTests
    {
        ...
        [Fact]
        public void JsCrypto_SubtleImportKey_KeyUsagesFractionalLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();

            var keyUsages = new FenObject();
            keyUsages.Set("length", FenValue.FromNumber(1.5));
            keyUsages.Set("0", FenValue.FromString("sign"));
            ...
            var result = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fractional-usage-length-key"))),
                    FenValue.FromObject(CreateAlgorithm("HMAC", "SHA-256")),
                    FenValue.FromBoolean(true),
                    FenValue.FromObject(keyUsages)
                },
        ...
        [Fact]
        public void JsCrypto_SubtleImportKey_KeyUsagesOversizedLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();

            var keyUsages = new FenObject();
            const int usageCount = 1025;
            keyUsages.Set("length", FenValue.FromNumber(usageCount));
            for (var i = 0; i < usageCount; i++)
            {
                keyUsages.Set(i.ToString(), FenValue.FromString("sign"));
            }
            ...
            var result = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("oversized-usage-length-key"))),
                    FenValue.FromObject(CreateAlgorithm("HMAC", "SHA-256")),
                    FenValue.FromBoolean(true),
                    FenValue.FromObject(keyUsages)
                },
        ...
        [Fact]
        public void JsCrypto_SubtleImportKey_ArrayLikeNonFiniteElement_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            ...
            keyData.Set("length", FenValue.FromNumber(3));
            keyData.Set("0", FenValue.FromNumber(1));
            keyData.Set("1", FenValue.FromNumber(double.PositiveInfinity));
            keyData.Set("2", FenValue.FromNumber(3));
        ...
        [Fact]
        public void JsCrypto_SubtleImportKey_ArrayLikeFractionalElement_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            ...
            keyData.Set("length", FenValue.FromNumber(3));
            keyData.Set("0", FenValue.FromNumber(1));
            keyData.Set("1", FenValue.FromNumber(2.5));
            keyData.Set("2", FenValue.FromNumber(3));
        ...
        [Fact]
        public void JsCrypto_SubtleImportKey_ArrayLikeNegativeElement_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            ...
            keyData.Set("length", FenValue.FromNumber(3));
            keyData.Set("0", FenValue.FromNumber(1));
            keyData.Set("1", FenValue.FromNumber(-1));
            keyData.Set("2", FenValue.FromNumber(3));
        ...
        [Fact]
        public void JsCrypto_SubtleGenerateHmacKey_FractionalLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();

            var algorithm = CreateAlgorithm("HMAC", "SHA-256");
            algorithm.Set("length", FenValue.FromNumber(256.5));
        ...
        [Fact]
        public void JsCrypto_SubtleGenerateRsaKeyPair_FractionalModulusLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();

            var algorithm = CreateAlgorithm("RSASSA-PKCS1-v1_5", "SHA-256");
            algorithm.Set("modulusLength", FenValue.FromNumber(1024.5));
            algorithm.Set("publicExponent", FenValue.FromObject(CreateArrayBuffer(new byte[] { 0x01, 0x00, 0x01 })));
        ...
        [Fact]
        public void JsCrypto_SubtleGenerateRsaKeyPair_OversizedModulusLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();

            var algorithm = CreateAlgorithm("RSASSA-PKCS1-v1_5", "SHA-256");
            algorithm.Set("modulusLength", FenValue.FromNumber(3_000_000_000d));
            algorithm.Set("publicExponent", FenValue.FromObject(CreateArrayBuffer(new byte[] { 0x01, 0x00, 0x01 })));
        ...
        [Fact]
        public void JsCrypto_SubtleGenerateRsaKeyPair_OversizedExponentWidth_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();
            ...
            var algorithm = CreateAlgorithm("RSASSA-PKCS1-v1_5", "SHA-256");
            algorithm.Set("modulusLength", FenValue.FromNumber(1024));
            algorithm.Set("publicExponent", FenValue.FromObject(CreateArrayBuffer(new byte[] { 0x00, 0x00, 0x01, 0x00, 0x01 })));
        ...
        [Fact]
        public void JsCrypto_SubtleRsaPss_SignWithNonFiniteSaltLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();
            var sign = subtle.Get("sign").AsFunction();
            ...
            var signAlgorithm = CreateAlgorithm("RSA-PSS");
            signAlgorithm.Set("saltLength", FenValue.FromNumber(double.PositiveInfinity));
            var signResult = sign.Invoke(
                new[]
                {
                    FenValue.FromObject(signAlgorithm),
                    FenValue.FromObject(privateKey),
                    FenValue.FromString("rsa-pss-nonfinite-salt-length")
                },
                null);
        ...
        [Fact]
        public void JsCrypto_SubtleGenerateAesGcmKey_NonFiniteLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();
        ...
        [Fact]
        public void JsCrypto_SubtleImportAesGcmKey_NonFiniteLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            ...
            var result = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(new byte[] { 0x12, 0x7A, 0xEE, 0x41, 0x99, 0x13, 0x8D, 0x05, 0xC0, 0x11, 0x22, 0x33, 0x67, 0x44, 0x10, 0xAF })),
                    FenValue.FromObject(algorithm),
                    FenValue.FromBoolean(true),
                    FenValue.FromObject(CreateStringArray("encrypt", "decrypt"))
                },
        ...
        [Fact]
        public void JsCrypto_SubtleImportAesGcmKey_FractionalLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
        ...
        [Fact]
        public void JsCrypto_SubtleEncrypt_AesGcmNonFiniteTagLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();
            var encrypt = subtle.Get("encrypt").AsFunction();
            ...
            var operationAlgorithm = CreateAlgorithm("AES-GCM");
            operationAlgorithm.Set("iv", FenValue.FromObject(CreateArrayBuffer(new byte[] { 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x40, 0x41 })));
            operationAlgorithm.Set("tagLength", FenValue.FromNumber(double.PositiveInfinity));
            ...
            var result = encrypt.Invoke(
                new[]
                {
                    FenValue.FromObject(operationAlgorithm),
                    FenValue.FromObject(key),
                    FenValue.FromString("aes-gcm-bad-tag-length")
                },
                null);
        ...
        [Fact]
        public void JsCrypto_SubtleEncrypt_AesGcmFractionalTagLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var generateKey = subtle.Get("generateKey").AsFunction();
            var encrypt = subtle.Get("encrypt").AsFunction();
            ...
            var operationAlgorithm = CreateAlgorithm("AES-GCM");
            operationAlgorithm.Set("iv", FenValue.FromObject(CreateArrayBuffer(new byte[] { 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x40, 0x41 })));
            operationAlgorithm.Set("tagLength", FenValue.FromNumber(96.5));
            ...
            var result = encrypt.Invoke(
                new[]
                {
                    FenValue.FromObject(operationAlgorithm),
                    FenValue.FromObject(key),
                    FenValue.FromString("aes-gcm-fractional-tag-length")
                },
                null);
        ...
        [Fact]
        public void JsCrypto_SubtleDeriveBits_Pbkdf2_FractionalLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            var deriveBits = subtle.Get("deriveBits").AsFunction();
            ...
            var baseKeyResult = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-password-material-fractional-len"))),
                    FenValue.FromObject(CreateAlgorithm("PBKDF2")),
                    FenValue.FromBoolean(false),
                    FenValue.FromObject(CreateStringArray("deriveBits"))
                },
        ...
        [Fact]
        public void JsCrypto_SubtleDeriveBits_Pbkdf2_OversizedLength_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            var deriveBits = subtle.Get("deriveBits").AsFunction();
            ...
            var baseKeyResult = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-password-material-oversized-len"))),
                    FenValue.FromObject(CreateAlgorithm("PBKDF2")),
                    FenValue.FromBoolean(false),
                    FenValue.FromObject(CreateStringArray("deriveBits"))
                },
            ...
            var deriveResult = deriveBits.Invoke(
                new[]
                {
                    FenValue.FromObject(deriveAlgorithm),
                    FenValue.FromObject(baseKey),
                    FenValue.FromNumber(3_000_000_000d)
                },
                null);
        ...
        [Fact]
        public void JsCrypto_SubtleDeriveBits_Pbkdf2_NonFiniteIterations_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            var deriveBits = subtle.Get("deriveBits").AsFunction();
            ...
            var baseKeyResult = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-password-material-nonfinite-iters"))),
                    FenValue.FromObject(CreateAlgorithm("PBKDF2")),
                    FenValue.FromBoolean(false),
                    FenValue.FromObject(CreateStringArray("deriveBits"))
                },
            ...
            var deriveAlgorithm = CreateAlgorithm("PBKDF2", "SHA-256");
            deriveAlgorithm.Set("salt", FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-salt-value"))));
            deriveAlgorithm.Set("iterations", FenValue.FromNumber(double.PositiveInfinity));
        ...
        [Fact]
        public void JsCrypto_SubtleDeriveBits_Pbkdf2_FractionalIterations_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            var deriveBits = subtle.Get("deriveBits").AsFunction();
            ...
            var baseKeyResult = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-password-material-fractional-iters"))),
                    FenValue.FromObject(CreateAlgorithm("PBKDF2")),
                    FenValue.FromBoolean(false),
                    FenValue.FromObject(CreateStringArray("deriveBits"))
                },
            ...
            var deriveAlgorithm = CreateAlgorithm("PBKDF2", "SHA-256");
            deriveAlgorithm.Set("salt", FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-salt-value"))));
            deriveAlgorithm.Set("iterations", FenValue.FromNumber(1000.5));
        ...
        [Fact]
        public void JsCrypto_SubtleDeriveBits_Pbkdf2_OversizedIterations_Rejects()
        {
            var subtle = GetSubtle(new JsCrypto());
            var importKey = subtle.Get("importKey").AsFunction();
            var deriveBits = subtle.Get("deriveBits").AsFunction();
            ...
            var baseKeyResult = importKey.Invoke(
                ...
                {
                    FenValue.FromString("raw"),
                    FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-password-material-oversized-iters"))),
                    FenValue.FromObject(CreateAlgorithm("PBKDF2")),
                    FenValue.FromBoolean(false),
                    FenValue.FromObject(CreateStringArray("deriveBits"))
                },
            ...
            var deriveAlgorithm = CreateAlgorithm("PBKDF2", "SHA-256");
            deriveAlgorithm.Set("salt", FenValue.FromObject(CreateArrayBuffer(Encoding.UTF8.GetBytes("fen-salt-value"))));
            deriveAlgorithm.Set("iterations", FenValue.FromNumber(3_000_000_000d));
