...
<html><body>
    <input id='date-invalid-lexical' type='date' min='2026-01-01' max='2026-12-31' value='06/01/2026' />
        ...
        [Fact]
        public void ValidityPseudoClasses_InvalidDateTimeLocalLexicalValue_IsInvalid()
        {
            var doc = Parse(@"
<!doctype html>
<html><body>
    <input id='dt-invalid-lexical' type='datetime-local' min='2026-01-01T00:00' max='2026-12-31T23:59' value='2026/06/01 12:30' />
</body></html>");

            var invalid = ById(doc, "dt-invalid-lexical");
            Assert.False(SelectorMatcher.Matches(invalid, ":valid"));
            Assert.True(SelectorMatcher.Matches(invalid, ":invalid"));
        }

        [Fact]
        public void ValidityPseudoClasses_InvalidMonthLexicalValue_IsInvalid()
        {
            var doc = Parse(@"
<!doctype html>
<html><body>
    <input id='month-invalid-lexical' type='month' min='2026-01' max='2026-12' value='2026/06' />
</body></html>");

            var invalid = ById(doc, "month-invalid-lexical");
            Assert.False(SelectorMatcher.Matches(invalid, ":valid"));
            Assert.True(SelectorMatcher.Matches(invalid, ":invalid"));
        }
...
<html><body>
    ...
    <a id='different-prefix' href='/docs/other/intro'>Different prefix</a>
        ...
        [Fact]
        public void LocalLinkPseudoClass_InvalidDepthArgument_IsRejected()
        {
            var doc = Parse(@"
<!doctype html>
<html><body>
    <a id='local' href='/docs/specs/intro'>Same prefix</a>
            ...
            var local = ById(doc, "local");
            Assert.False(SelectorMatcher.Matches(local, ":local-link(foo)"));
            Assert.False(SelectorMatcher.Matches(local, ":local-link(-1)"));
        }
...
<html data-active-view-transition='true'><body>
    <div id='target'></div>
        ...
        [Fact]
        public void ActiveViewTransitionTypePseudoClass_MatchesDeclaredTransitionType()
        {
            var doc = Parse(@"
<!doctype html>
<html data-active-view-transition='true' data-active-view-transition-types='navigation swap'><body>
    <div id='target'></div>
            ...
            Assert.True(SelectorMatcher.Matches(target, ":active-view-transition-type(navigation)"));
            Assert.False(SelectorMatcher.Matches(target, ":active-view-transition-type(reveal)"));
        }
